...
def _user_from_api_key(x_api_key: str, db: Session) -> Optional[models.User]:
    # Récupérer tous les agents actifs pour comparer la clé reçue
    agents = (
        db.query(models.User)
        .filter(models.User.user_type == "AGENT", models.User.is_active == True)
        .all()
    )
    matched_user = None
    for user in agents:
        ...
        # Si la clé stockée commence par $2, c'est un hash bcrypt
        if stored_key.startswith("$2"):
            if security.verify_password(x_api_key, stored_key):
                matched_user = user
                break
        else:
            # Rétrocompatibilité : clé en clair
            if stored_key == x_api_key:
                # Migration douce : hashage à la volée pour sécuriser immédiatement la clé
                user.api_key = security.get_password_hash(x_api_key)
                db.commit()
                matched_user = user
                break
    ...
    exp = getattr(matched_user, "api_key_expires_at", None)
    if exp is not None:
