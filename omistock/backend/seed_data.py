...
def seed(admin_only=False):
    ...
    try:
        ...
        s_saidal = models.Supplier(
            name="Saidal Group", email="contact@saidal.dz", company_id=company.id
        )
        s_biopharm = models.Supplier(
            name="Biopharm", email="info@biopharm.com", company_id=company.id
        )
        db.add_all([s_saidal, s_biopharm])
        db.commit()
        db.refresh(s_saidal)
        # 6. Création des Produits Pharma / Parapharma pour la démo principale
