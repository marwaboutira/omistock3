...
def create_agent(db: Session, agent_data: DataDict, company_id: int) -> models.User:
    try:
        ...
        hashed_key = security.get_password_hash(data["api_key"])
...
def create_supplier(db: Session, supplier_data: DataDict, company_id: int) -> models.Supplier:
    """Crée un fournisseur pour l'entreprise courante."""
    try:
        d = _to_dict(supplier_data)
        if not d.get("name"):
...
def get_agent_proposals(
    ...
    if agent_id is not None:
        q = q.filter(models.AgentProposal.agent_id == agent_id)
...
# =============================================================================
# Bons de commande (Purchase Orders) — Cycle DRAFT -> SENT -> RECEIVED
# =============================================================================

def _generate_order_number(db: Session, company_id: int) -> str:
    """Génère un numéro de BC unique basé sur le compteur courant."""
    count = db.query(models.PurchaseOrder).filter(
        models.PurchaseOrder.company_id == company_id
    ).count()
    return f"BC-{company_id:03d}-{count + 1:05d}"


def get_purchase_orders(
    db: Session, company_id: int, status: Optional[str] = None
) -> List[models.PurchaseOrder]:
    q = db.query(models.PurchaseOrder).filter(models.PurchaseOrder.company_id == company_id)
    if status:
...
def get_purchase_order_by_id(
    ...
    return (
        ...
        .filter(models.PurchaseOrder.id == po_id, models.PurchaseOrder.company_id == company_id)
        .first()
    )
...
def create_purchase_order(
    ...
    *,
    ...
    """Crée un BC en statut DRAFT avec ses lignes. Ne touche pas au stock."""
    try:
        order_number = _generate_order_number(db, company_id)
        ...
        po = models.PurchaseOrder(
            ...
            company_id=company_id,
            creator_id=actor_id,
            status=models.OrderStatus.DRAFT.value,
...
def send_purchase_order(
    db: Session, po_id: int, company_id: int, actor_id: int
) -> models.PurchaseOrder:
    """Transition DRAFT -> SENT. Ne touche pas au stock."""
    try:
        po = get_purchase_order_by_id(db, po_id, company_id)
        if not po:
            raise ValueError("Bon de commande introuvable.")
        if po.status != models.OrderStatus.DRAFT.value:
            raise ValueError(f"Seul un BC en brouillon peut être envoyé (statut actuel: {po.status}).")
        po.status = models.OrderStatus.SENT.value
        db.commit()
        db.refresh(po)
        return po
    except ValueError:
...
def receive_purchase_order(
    ...
    """Transition SENT -> RECEIVED. Appelle restock_product pour chaque ligne (WAC + stock)."""
    from datetime import datetime, timezone
    try:
        po = get_purchase_order_by_id(db, po_id, company_id)
        if not po:
            raise ValueError("Bon de commande introuvable.")
        if po.status != models.OrderStatus.SENT.value:
            raise ValueError(f"Seul un BC envoyé peut être reçu (statut actuel: {po.status}).")
        for item in po.items:
            restock_product(
                ...
                reason=f"Réception BC {po.order_number}",
            )
...
def cancel_purchase_order(
    ...
    """Annulation depuis DRAFT ou SENT. Ne touche pas au stock."""
    try:
        po = get_purchase_order_by_id(db, po_id, company_id)
        if not po:
            raise ValueError("Bon de commande introuvable.")
        if po.status not in (models.OrderStatus.DRAFT.value, models.OrderStatus.SENT.value):
            raise ValueError(f"Impossible d'annuler un BC en statut '{po.status}'.")
        po.status = models.OrderStatus.CANCELLED.value
        db.commit()
        db.refresh(po)
        return po
    except ValueError:
