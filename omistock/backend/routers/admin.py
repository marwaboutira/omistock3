...
async def restore_database(
    ...
    try:
        ...
        db.query(models.PurchaseOrderItem).filter(
            models.PurchaseOrderItem.order_id.in_(
...
def export_logs(
    ...
    output = io.StringIO()
    output.write("﻿")  # BOM UTF-8 : Excel affiche correctement les accents
    writer = csv.writer(output, delimiter=";")  # ';' : séparateur attendu par Excel (locale FR)
    writer.writerow(["ID", "Date/Heure", "Utilisateur (ID)", "Action", "Ancienne valeur", "Nouvelle valeur"])
...
def export_report(
    ...
    """Export du rapport de stock au format CSV (compatible Excel).
    Inclut, par produit : stock total, point de commande (ROP), prix de vente,
    coût moyen pondéré (WAC) et valeur du stock au coût."""
    products = repository.get_products(db, current_user.company_id)

    output = io.StringIO()
    output.write("﻿")  # BOM UTF-8 : Excel affiche correctement les accents
    writer = csv.writer(output, delimiter=";")  # ';' : séparateur attendu par Excel (locale FR)
    writer.writerow([
        "Produit", "SKU", "Stock total", "Point de commande (ROP)",
        "Prix de vente", "Cout unitaire (WAC)", "Valeur stock (au cout)",
    ])
    for p in products:
        qty = p.total_quantity
        cost = round(p.cost_price or 0.0, 2)
        writer.writerow([
            p.name, p.sku or "", qty, p.reorder_point,
            round(p.price or 0.0, 2), cost, round(qty * cost, 2),
        ])
    output.seek(0)
    ...
    return StreamingResponse(
        iter([output.getvalue()]),
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=rapport_omistock.csv"},
    )
...
# @deprecated : alias de l'ancien chemin /api/mcp/chat (interface HUMAINE, pas le serveur MCP).
# Conservé pour ne casser aucun appel ; à retirer après migration complète du frontend.
@router.post("/api/mcp/chat", deprecated=True)
...
def approve_agent_proposal(
    ...
    try:
        if prop.action_type == "RESTOCK":
        ...
        elif prop.action_type == "REVERSE":
            payload = json.loads(prop.payload)
            sale = repository.reverse_sale(
                db, sale_id=payload["sale_id"], company_id=current_user.company_id,
                actor_id=current_user.id, correlation_id=corr,
            )
            result_entity = sale.id
        elif prop.action_type == "TRANSFER":
            payload = json.loads(prop.payload)
            # Crée la demande de transfert, puis l'approuve ET la confirme atomiquement
            # sous l'identité de l'admin (SoD : l'agent ne touche jamais au stock directement).
            ...
            repository.approve_transfer_request(db, req.id, current_user.id)
            repository.confirm_transfer_request(db, req.id, current_user.id)
            result_entity = req.id
        else:
