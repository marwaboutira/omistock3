...
# @deprecated : alias de l'ancien chemin /api/mcp/analyze (interface HUMAINE, pas le serveur MCP).
# Conservé pour ne casser aucun appel ; à retirer après migration complète du frontend.
@router.post("/api/mcp/analyze", deprecated=True)
...
def create_supplier_route(
    ...
    try:
        return repository.create_supplier(db, data.model_dump(), current_user.company_id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
