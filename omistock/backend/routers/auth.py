    ...
    return [
        {
            ...
            "api_key_masked": "********" if (a.api_key and a.api_key.startswith("$2")) else ((a.api_key[:4] + "…" + a.api_key[-3:]) if a.api_key else None),
...
def rotate_agent_key(
    ...
    agent.api_key = security.get_password_hash(new_key)
...
def revoke_agent(
    ...
    """Révoque un agent : désactive son accès (sa clé cesse immédiatement de fonctionner).
    On ne supprime pas la ligne (l'historique d'audit est conservé)."""
    agent = (
        db.query(models.User)
        .filter(models.User.id == agent_id, models.User.company_id == current_user.company_id,
                models.User.user_type == "AGENT")
    ...
    db.commit()
    corr = getattr(request.state, "correlation_id", None) or audit.new_correlation_id()
    audit.record(db, user_id=current_user.id, actor_type="ADMIN", action="AGENT_REVOKED",
                 company_id=current_user.company_id, entity_type="agent", entity_id=agent.id,
                 correlation_id=corr)
